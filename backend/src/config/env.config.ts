...
const envSchema = z.object({
  ...
  CHAT_MODEL: z.string().default("llama-3.3-70b-versatile"),
  ...
  WORKSPACE_DAILY_TOKEN_BUDGET: z
    .string()
    .default("200000")
    .transform((val) => parseInt(val, 10)),
  WORKSPACE_INGESTION_CONCURRENCY: z
    .string()
    .default("2")
    .transform((val) => parseInt(val, 10)),
  LOG_LEVEL: z
...
export const config = {
  ...
  chatModel: env.CHAT_MODEL || "llama-3.3-70b-versatile",
...
function assertProductionConfig(): void {
  ...
  const servesHttpApi =
    config.processRole === "api" || config.processRole === "all";
  if (servesHttpApi) {
...
function assertEmbeddingConfig(): void {
  ...
  const hasEmbeddingKey =
    Boolean(jinaKey) ||
    Boolean(config.openaiApiKey && !config.openaiApiKey.startsWith("mock-"));

  if (config.nodeEnv === "production" && !hasEmbeddingKey) {
    throw new Error(
      "Fatal: Production requires JINA_API_KEY or OPENAI_API_KEY for embeddings.",
    );
  }
}
...
if (
  ...
  console.warn(
    "⚠️ WARNING: OPENAI_API_KEY / GROQ_API_KEY environment variable is not set!",
  );
}
